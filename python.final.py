...
def run():
    ...
    for i in result: #for loop to check the input query is in database or not
        if i[0] == mqttQuery: #if the i is same as mqtt query
            ...
            res = i[1]
    if not res:
        print("key is not found") #print if data is not found
    publisher(client, res)
    client.loop_stop()

#for importing the data from the database created
cnx = pymysql.connect(user="root", passwd="1234", host="localhost", database="remote")  #define the mysql user, pwd, host, database
mycursor = cnx.cursor()
